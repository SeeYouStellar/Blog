...
1. 读写并不能瞬间完成，而是在一个时间段内进行。
2. 读在写开始前完成，读到的一定是旧值；读在写完成之后开始，读到的一定是新值。(读写操作无重合部分)
3. 读写并发进行，即有重合部分时，既可能读到新值，也可能读到旧值。
4. 一旦有一个客户端读取到了新值，那么之后的客户端一定也都会读取到新值。
...
**预写日志+二阶段提交+多数派原则保证了被提交日志的最终一致性语义，而日志被提交后，也必定按顺序被状态机应用，所以状态机的数据也满足最终一致性。**
#### 1. 顺序一致性（满足）
...
#### 3.即时一致性（不满足）
即时一致性指的是：客户端发起请求到客户端收到服务端发送的响应之后，若客户端还是无法读到正确的值。

一般来说，服务端的（数据同步）与（响应客户端）这两个操作是异步完成的话，就肯定无法满足即时一致性，因为响应客户端操作很明显会快于数据同步，那么在还没同步完之前客户端就收到了响应，收到响应就以为数据已同步完成，显然会出现数据不一致的问题。

在我的实现时（mit6.824），raft 的 server 层收到了预写日志提交的消息后，就将日志写入状态机，随后再响应客户端，两个操作是同步的。这样保证了如果客户端访问的是leader节点，那么即时一致性是可以满足的。但是实际的raft论文里描述的是，基于raft的应用，读操作可由任意节点提供服务，那么倘若一个存在数据状态滞后的 follower 提供了服务，那么客户端就可能读到老数据. 因此raft 无法满足即时一致性.。

**这里又有一个矛盾，这种数据同步的情况是raft遵从多数派原则导致的。但如果不使用多数派原则，系统的高可用性将大大降低。**

为了满足即时一致性，尝试两种方式：
1. **强制读主策略**：也就是客户端只能读取leader节点的数据，这种策略看似可以保证数据的即时一致性，其实也不能。当集群出现网络分区时，出现网络问题的leader有可能被某个客户端认为是leader，而真正的leader的状态机状态早就更新好几次了，那么这个客户端访问到的内容就是过时的。
	要想在强制读主的基础上，满足即时一致性，需要在leader提供读服务时（比如server接收到读请求时、server响应客户端时。。）发起广播（内容无所谓），得到多数server的响应时就可以提供读服务了，这一操作保证该leader是真正的leader。
2. **appliedIndex** **校验**：每次 leader 处理写请求后，会把最晚一笔应用到状态机的日志索引 appliedIndex 反馈给客户端. 后续客户端和 follower 交互时，会携带 appliedIndex. 倘若 follower 发现自身的 appliedIndex 落后于客户端的 appliedIndex，说明本机存在数据滞后，则拒绝这笔请求，由客户端发送到其他节点进行处理.

### 四、raft为什么无法保证高可用？

raft中的请求都是两阶段提交的。leader接受到请求后，需将请求同步到所有follower中，再通过follower的响应来判断该请求是否已经同步到半数以上follower中，从而确定该条请求可提交，通知状态机应用。故若**集群中大于半数的server宕机**则这条请求无法应用到状态机中，客户端也就无法收到状态机传来的该条请求被应用的ack消息，那么客户端不可避免会等待（具体实现时等待只会持续一段时间），但这段等待的时间**客户端无法即时收到响应，就不算高可用了**

多数派原则是raft提高高可用的一种措施。
...
4. **Leader Completeness**：如果某个日志已经在某个term被leader commit，那么该日志也会出现在之后的term的leader的日志中。即每个leader的日志中都保留了之前所有已经commit的日志。
    ...
    [外链图片转存失败,源站可能有防盗链机制,建议将图片保存下来直接上传(img-yJXKKHCw-1692431877349)(image/image1.png)]
...
[外链图片转存失败,源站可能有防盗链机制,建议将图片保存下来直接上传(img-QvjLf2NQ-1692431877350)(image/image2.png)]
